...
class StorageService {
  ...
  get confirmed(): boolean {
    if (
      ...
      return (
        StorageService.getInstance().storage.getBoolean(
          StorageToken.Confirmed,
        ) ?? false
      );
    }
    return false;
  ...
  get token(): Token {
    if (
      ...
      const tokenStr = StorageService.getInstance().storage.getString(
        StorageToken.Token,
      );
      try {
  ...
  get userInfo(): UserInfo {
    if (
      ...
      const userInfoStr = StorageService.getInstance().storage.getString(
        StorageToken.UserInfo,
      );
      try {
        ...
        case 'object':
          ...
          if (oldValue) {
            ...
            StorageService.getInstance().storage.set(
              key,
              JSON.stringify(removeEmpty({ ...oldObj, ...value })),
            );
          } else {
            StorageService.getInstance().storage.set(
              key,
              JSON.stringify(removeEmpty({ ...(value as Obj) })),
            );
          }
          break;
